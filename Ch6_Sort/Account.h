#pragma once
#include <iostream>
#include<iomanip>
#include<string>
#include<utility>
#include "Transaction.h"
using first_name = std::string;
using second_name = std::string;
using Name = std::pair<first_name, second_name>;

class Account {
private:
	size_t account_number;
	Name name{};
	double balance;
public:
	Account() = default;
	Account(size_t number, const Name& nm) : account_number{ number }, name{ nm }{}

	void set_balance(double bal) { balance = bal; }
	double get_balance() const { return balance; }
	const Name& get_name() const { return name; }
	size_t get_acc_number(){ return account_number; }

	bool apply_transaction(const Transaction& tr) {
		if (tr.credit) balance += tr.amount;
		else balance -= tr.amount;
		return balance < 0.0;
	}

	bool operator<(const Account& acc) const { return account_number < acc.account_number; }
	friend std::ostream& operator<<(std::ostream& out, const Account& account);

};

std::ostream& operator<<(std::ostream& out, const Account& account) {
	out << std::left << std::setw(20) << account.name.first + " " << account.name.second <<
		std::right << std::setfill('0') << std::setw(5) << account.account_number << std::setfill(' ')
		<< std::setw(8) << std::fixed << std::setprecision(2) << account.balance;
}

class Compare_Name {
public:
	bool operator()(const Account& acc1, const Account& acc2)
	{
		const Name& name1 = acc1.get_name(); const Name& name2 = acc2.get_name();
		return name1.second < name2.second || (name1.second == name2.second && name1.first < name2.first);
	}
};